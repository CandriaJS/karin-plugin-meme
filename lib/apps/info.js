import {e as e$1}from'../chunk-7WFRRIXH.js';import {e}from'../chunk-AAQCRG3Q.js';import I,{segment,base64,logger}from'node-karin';var b=I.command(/^#?(?:(?:柠糖)?表情)详情\s*(.+)$/i,async p=>{try{if(!e.meme.enable)return !1;let[,a]=p.msg.match(b.reg),f=await e$1.get_meme_info_by_keyword(a)??await e$1.get_meme_info(a);if(!f)throw new Error(`\u6CA1\u6709\u627E\u5230\u8BE5\u8868\u60C5${a}\u4FE1\u606F`);let{key:y,keyWords:s,min_images:g,max_images:c,min_texts:l,max_texts:x,default_texts:o,options:i,tags:m}=f,$=await e$1.get_preset_all_about_keywords_by_key(y),T=typeof s=="string"?JSON.parse(s):Array.isArray(s)?s:[],_=typeof o=="string"?JSON.parse(o):Array.isArray(o)?o:[],A=typeof m=="string"?JSON.parse(m).map(e=>`[${e}]`):Array.isArray(m)?m:[],h=typeof i=="string"?JSON.parse(i):Array.isArray(i)?i:[],u=h.length>0?h.map(e=>`[${e.name}: ${e.description}]`).join(""):null,w=Array.isArray($)?$.map(e=>`[${e}]`).join(" "):null,r=[segment.text(`\u540D\u79F0: ${y}
...
\u53EF\u9009\u9009\u9879:
${u}`));try{let e=await e$1.get_meme_preview(y);e&&(r.push(segment.text(`
\u9884\u89C8\u56FE\u7247:
