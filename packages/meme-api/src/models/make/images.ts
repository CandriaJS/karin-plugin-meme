...
> {
  ...
  const getType = isRust ? 'url' : 'buffer'
  const AvatarUploadType = 'url'
  ...
  if (isRust) {
    const imagePromises = messageImages.map(async (msgImage) => {
      const [image, name] = await Promise.all([
        ...
        utils.getUserAame(e, msgImage.userId),
      ])
      return {
        name,
        id: image,
  ...
  if (allUsers.length > 0) {
    let avatar = await utils.getUserAvatar(e, allUsers[0], getType)
    if (!avatar) {
      return {
        success: false,
        message: '获取用户头像失败',
    ...
    if (isRust) {
      const image = await utils.upload_image({
        image: avatar.avatar,
        type: AvatarUploadType,
      })
      if (image) {
        userAvatars.push({
          name: await utils.getUserAame(e, avatar.userId),
          id: image,
        })
      }
    } else {
  ...
  if (messageImages.length === 0 && quotedUser) {
    let avatar = await utils.getUserAvatar(e, quotedUser, getType)
    if (!avatar) {
      return {
        success: false,
        message: '获取用户头像失败',
  ...
  if (min_images === 1 && messageImages.length === 0) {
    let avatar = await utils.getUserAvatar(e, e.userId, getType)
    if (!avatar) {
      return {
        success: false,
        message: '获取用户头像失败',
  ...
  if (isRust) {
    images = [...userAvatars, ...images].slice(0, max_images) as Array<{
      name: string
      id: string
    }>
  } else {
